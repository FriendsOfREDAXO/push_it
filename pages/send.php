...
if ($doSend) {
    ...
    if (!$isAdmin && ($formData['icon'] || $formData['badge'] || $formData['image'])) {
        echo rex_view::warning(rex_i18n::msg('admin_images_only'));
    }
}
...
$statsFragment->setVar('title', rex_i18n::msg('current_subscribers_title'), false);
...
if (!empty($formData['title']) || !empty($formData['body'])) {
    ...
    if ($previewContent) {
        ...
        $previewFragment->setVar('title', rex_i18n::msg('preview_title'), false);
...
$formFragment->setVar('title', rex_i18n::msg('send_notification_title'), false);
